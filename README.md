...
Плагін не радить. Він констатує факт і мовчить далі. Кожне речення, яке він здатен вимовити, є константою-шаблоном у `hooks/sincelast.py`; вільно згенерованого тексту немає взагалі.

## Чому факт, а не таймстемп

**Мовчить, поки не має чого сказати.** Нічого не змінилось, отже нічого не вставляється. Контекст лишається для роботи, а не для годинника.
...
**Каже лише те, що перевірив.** Твердження про предків і лічильник комітів з'являються лише коли гілка не змінювалась. Звичайний `checkout` на розбіжну гілку дає «позиція змінилась» і жодних здогадів про історію.
...
- **bash обовʼязковий, включно з Windows.** Команда хука написана синтаксисом bash: `"shell": "bash"` і завершальне `|| true`. Під PowerShell цей хвіст означає інше, і гарантія м'якого падіння втрачається. На Windows користуйся bash із Git for Windows.
...
Дві речі, які варто знати (обидві спостережені, а не припущені):
...
**Нічого не залишає цю машину.** Прочитане й збережене не виходить за межі машини: жодних мережевих викликів, жодних лог-файлів.
...
|---|---|---|
| `SINCELAST_DISABLE` | не задано | будь-яке значення, крім порожнього, `0` чи `false`, вимикає плагін цілком; він виходить до того, як щось прочитає |
| `SINCELAST_DEBUG` | не задано | при внутрішньому збої пише в stderr тип винятку, рядок і назву події; ніколи payload чи локальні змінні |
| `XDG_STATE_HOME` | `~/.local/state` | батьківська тека для файлів стану `sincelast/` |

## Часовий пояс

Дата **машинна** і збігається з григоріанською датою, яку несе системний промпт. Рахується в Python, а не через `date(1)`, чий локале-чутливий вивід (тайсько-буддійський календар, японська ера) з нею не збігся б.

Dev-контейнер, що працює в UTC (звичний дефолт, бо зону хоста він не успадковує), зсуває момент зміни дати: розробник у Берліні побачить повідомлення о 02:00 за місцевим часом. Лікується на рівні контейнера: `TZ=Europe/Berlin` або монтування `-v /etc/localtime:/etc/localtime:ro`.

## Поведінка при збої

Кожен шлях відмови веде в тишу. Модуль обгорнутий у `try/except BaseException` (включно з імпортами), команда хука завершується `|| true`, а процес завжди виходить із кодом `0`, бо ненульовий вихід зі `Stop` заблокував би агента. За будь-якої помилки stdout порожній.
...
Unit-тести працюють офлайн і не створюють файлів поза `tmp_path`.
...
Він покриває покручені входи, git що завжди падає, git якого нема,
